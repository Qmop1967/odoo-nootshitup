...
class EnhancedContinuousSyncServer:
    def __init__(self):
        ...
        # Email configuration
        self.notification_email = "kha89ahm@gmail.com"
        self.smtp_config = {
    ...
    def send_email_notification(self, subject, body):
        ...
        try:
            ...
            logger.info(f"✉️ Email notification sent to {self.notification_email}")
        except Exception as e:
            logger.error(f"❌ Failed to send email notification: {e}")

    def prepare_sync_report(self, sync_record):
        """Prepare detailed sync report for email"""
        report = []
        report.append("🔄 ZOHO-ODOO SYNC REPORT")
        report.append("=" * 50)
        report.append(f"Sync Time: {sync_record['timestamp']}")
        report.append(f"Duration: {sync_record['duration']:.1f} seconds")
        report.append("\n📦 PRODUCT SYNCHRONIZATION:")
        report.append(f"- Zoho Products: {sync_record['zoho_product_count']}")
        report.append(f"- Odoo Products: {sync_record['odoo_product_count']}")
        report.append(f"- Products Added: {sync_record['products_added']}")
        report.append(f"- Products Updated: {sync_record['products_updated']}")
        report.append(f"- Product Errors: {sync_record['products_errors']}")
        report.append("\n💰 PRICE LIST SYNCHRONIZATION:")
        report.append(f"- Zoho Price Lists: {sync_record['zoho_pricelist_count']}")
        report.append(f"- Odoo Price Lists: {sync_record['odoo_pricelist_count']}")
        report.append(f"- Price Lists Added: {sync_record['pricelists_added']}")
        report.append(f"- Price Lists Updated: {sync_record['pricelists_updated']}")
        report.append(f"- Price List Items Added: {sync_record['pricelist_items_added']}")
    ...
    def sync_complete_exact_copy(self):
        ...
        try:
            ...
            report = self.prepare_sync_report(sync_record)
            subject = "✅ Zoho-Odoo Sync Completed Successfully"
            if product_errors > 0:
                subject = "⚠️ Zoho-Odoo Sync Completed with Errors"
            ...
            error_report = f"❌ Sync Failed\n\nError: {str(e)}\n\nTime: {datetime.now().isoformat()}"
            self.send_email_notification("❌ Zoho-Odoo Sync Failed", error_report)
