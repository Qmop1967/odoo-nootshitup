...
class ZohoOdooSyncServiceWithImages:
    """
    Enhanced one-way synchronization service from Zoho Books to Odoo
    ...
    def __init__(self, config_path='/opt/odoo/migration/config/zoho_config.json'):
        ...
        self.logger.info("🚀 Zoho-Odoo Sync Service with Images initialized successfully")
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(f'{log_dir}/sync_service_images.log'),
                logging.StreamHandler(sys.stdout)
            ]
        )
        
        self.logger = logging.getLogger('ZohoOdooSyncImages')
    ...
    def setup_directories(self):
        ...
        self.tracking_file = f'{self.data_dir}/sync_tracking_images.json'
    ...
    def save_sync_tracking(self):
        ...
        try:
            ...
            if os.path.exists(self.tracking_file):
                ...
                backups = [f for f in os.listdir(backup_dir) if f.startswith('sync_tracking_images.json.backup_')]
                if len(backups) > 5:
                    backups.sort()
    ...
    def download_product_image(self, image_url: str, zoho_id: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.error(f"❌ Error downloading image for product {zoho_id}: {e}")
            return None
    ...
    def upload_image_to_odoo(self, local_path: str, product_id: int, zoho_id: str) -> Optional[int]:
        ...
        try:
            ...
            self.logger.error(f"❌ Error uploading image to Odoo for product {product_id}: {e}")
    ...
    def sync_product_image(self, zoho_product: dict, odoo_product_id: int) -> bool:
        ...
        try:
            ...
            image_url = zoho_product.get('image_url') or zoho_product.get('image_document_id')
    ...
    def start_service(self):
        ...
        self.running = True
        self.logger.info(f"🚀 Starting Zoho-Odoo Sync Service with Images (interval: {self.sync_interval}s)")
