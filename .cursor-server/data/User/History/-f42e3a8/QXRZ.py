...
def connect_to_odoo(self):
    ...
    def create_zoho_field(self):
        ...
        try:
            ...
            model_ids = self.odoo_models.execute_kw(
                self.odoo_config['database'],
                self.odoo_uid,
                self.odoo_config['password'],
                'ir.model',
            ...
            if not model_ids:
                raise Exception("Could not find product.template model")
            ...
            field_id = self.odoo_models.execute_kw(
                self.odoo_config['database'],
                self.odoo_uid,
                self.odoo_config['password'],
                'ir.model.fields',
            ...
            if field_id:
                self.logger.info("✅ Created Zoho Item ID field")
                return True
            
            return False
            
        except Exception as e:
            self.logger.error(f"❌ Error creating Zoho field: {e}")
            return False 