...
def test_zoho_books(access_token, config):
    """Test Zoho Books API"""
    ...
    try:
        ...
        url = f'{config["base_url"]}/organizations/{config["organization_id"]}'
        ...
        if response.status_code == 200:
            ...
            contacts_url = f'{config["base_url"]}/contacts?organization_id={config["organization_id"]}&per_page=1'
...
def test_zoho_inventory(access_token, config):
    ...
    try:
        # Test items endpoint
        url = f'{config["base_url"]}/items?organization_id={config["organization_id"]}&per_page=1'
        response = requests.get(url, headers=headers)
...
def main():
    ...
    if books_access_token:
        books_success = test_zoho_books(books_access_token, books_config)
    else:
    ...
    if inventory_access_token:
        inventory_success = test_zoho_inventory(inventory_access_token, inventory_config)
    else:
